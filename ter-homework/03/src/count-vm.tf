...
resource "yandex_compute_instance" "web" {
  ...
  name        = "${var.vm_web[0].name}-${count.index + 1}" #Имя ВМ в облачной консоли
  hostname    = "${var.vm_web[0].name}-${count.index + 1}" #формирует FDQN имя хоста, без hostname будет сгенрировано случаное имя.
